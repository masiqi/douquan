...
def get_media_internal (request, path, **kwargs) :
	...
	my_path = path
	if kwargs['height'] is not None and kwargs['width'] is not None:
	    my_path = my_path + '?height=' + kwargs['height'] + '&width=' + kwargs['width']
	if kwargs['height'] is not None and kwargs['width'] is None:
	    my_path = my_path + '?height=' + kwargs['height']
	if kwargs['height'] is None and kwargs['width'] is not None:
	    my_path = my_path + '?width=' + kwargs['width']
	if not os.path.isfile(my_path):
	    f = file(my_path, "wb")
	    f.write(h.render().read())
	    f.close()
	    h.render().seek(0)
