...
# REST URLs
CHINAMOBILE_URL = 'http://jiekou.shequ.10086.cn/restserver.php'
...
class Chinamobile(object):
    ...
    def _build_post_args(self, method, args=None):
        ...
        args['mi_sig'] = self._hash_args(args)
...
#        logging.debug('calling api with data: %s' % post_data)
        ...
        if self.proxy:
            ...
            if secure:
                response = opener.open(CHINAMOBILE_URL, post_data).read()
            else:
                response = opener.open(CHINAMOBILE_URL, post_data).read()
        else:
            if secure:
                response = urlread(CHINAMOBILE_URL, post_data)
            else:
                response = urlread(CHINAMOBILE_URL, post_data)
	...
	if not request.GET.get('mi_sig_session_key'):
	    return False
	else:
	    self.session_key = request.GET.get('mi_sig_session_key')
	    return True
    ...
    def validate_signature(self, post, prefix='mi_sig', timeout=None):
        """
